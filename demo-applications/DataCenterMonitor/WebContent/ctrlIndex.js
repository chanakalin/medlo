abdcapp.controller('ctrlIndex', function($scope,$rootScope,$http) {

	
	$scope.medlonode = "r17s2a0l419e";
	...
	$scope.generateCharts = function($fromEpoch,$toEpoch){
		console.log("Loading medlo node data of "+$scope.medlonode);				
		$userName="admin";
		$passWord="admin";
		$http({
			method 	: "POST",
			...
			data	: JSON.stringify({ applicationName:"appName", tags:["device_management"]})
				...
				$http({
					...
					$http({
						method 	: "GET",
						url		: "https://medlo.trync.org:8243/medlonode/1.0.0/device/stats/"+$scope.medlonode+"?sensorType=temperature&from="+$fromEpoch+"&to="+$toEpoch,
						headers	: {
							   "Access-Control-Allow-Origin": "*",
						...
						responseType	: "json"
							...
							Highcharts.chart('chartTemperature', {
							    ...
							    subtitle: {text: 'Temperature data from medlo node '+$scope.medlonode},
							    xAxis: {
							        type: 'datetime',
					...
					$http({
						method 	: "GET",
						url		: "https://medlo.trync.org:8243/medlonode/1.0.0/device/stats/"+$scope.medlonode+"?sensorType=humidity&from="+$fromEpoch+"&to="+$toEpoch,
						headers	: {
							   "Access-Control-Allow-Origin": "*",
							   "Access-Control-Allow-Headers": "authorization,Access-Control-Allow-Origin",
							   "authorization": "Bearer "+$rootScope.wso2iotAccessToken
						...
						responseType	: "json"
							...
							Highcharts.chart('chartHumidity', {
							    chart: {type: 'spline'},
							    title: {text: 'Humidity'},
							    subtitle: {text: 'Humidity data from medlo node '+$scope.medlonode},
							    xAxis: {
							        type: 'datetime',
							        dateTimeLabelFormats: { // don't display the dummy year
							            month: '%e. %b',
