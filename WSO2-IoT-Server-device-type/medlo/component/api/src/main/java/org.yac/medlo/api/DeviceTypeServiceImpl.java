...
package org.yac.medlo.api;

import org.json.JSONObject;
import org.yac.medlo.api.dto.DeviceJSON;
import org.yac.medlo.api.dto.SensorRecord;
import org.yac.medlo.api.util.APIUtil;
import org.yac.medlo.api.util.ZipUtil;
import org.yac.medlo.api.util.ZipArchive;
import org.yac.medlo.plugin.constants.DeviceTypeConstants;
...
public class DeviceTypeServiceImpl implements DeviceTypeService {
     ...
     */
    @Path("device/{deviceId}/configranges")
    @POST
    public Response changeStatus(@PathParam("deviceId") String deviceId,
                                 @QueryParam("max-temperature") String maxTemperature,
                                 @QueryParam("min-temperature") String minTemperature,
                                 @QueryParam("max-humidity") String maxHumidity,
                                 @QueryParam("min-humidity") String minHumidity,
        ...
        try {
            ...
            /**
             * Validate and build payload as a JSON object
             */
            JSONObject payloadObject = new JSONObject();
            float checkValues =(float)0.0;
            try{
                checkValues = Float.parseFloat(maxTemperature);
                payloadObject.put("maxtemperature",checkValues);
            }catch(Exception e){
                log.error("Maximum temperature value is not valid '"+maxTemperature+"'");
                return Response.status(Response.Status.BAD_REQUEST.getStatusCode()).build();
            }
            try{
                checkValues = Float.parseFloat(minTemperature);
                payloadObject.put("mintemperature",checkValues);
            }catch(Exception e){
                log.error("Minimum temperature value is not valid '"+minTemperature+"'");
                return Response.status(Response.Status.BAD_REQUEST.getStatusCode()).build();
            }
            try{
                checkValues = Float.parseFloat(maxHumidity);
                payloadObject.put("maxhumidity",checkValues);
            }catch(Exception e){
                log.error("Maximum humidity value is not valid '"+maxHumidity+"'");
                return Response.status(Response.Status.BAD_REQUEST.getStatusCode()).build();
            }
            try{
                checkValues = Float.parseFloat(minHumidity);
                payloadObject.put("minhumidity",checkValues);
            }catch(Exception e){
                log.error("Minimum humidity value is not valid '"+minHumidity+"'");
                return Response.status(Response.Status.BAD_REQUEST.getStatusCode()).build();
            }
            ...
            String publishTopic = APIUtil.getAuthenticatedUserTenantDomain()
                    + "/" + DeviceTypeConstants.DEVICE_TYPE + "/" + deviceId + "/configranges";
            dynamicProperties.put(DeviceTypeConstants.ADAPTER_TOPIC_PROPERTY, publishTopic);
            Operation commandOp = new CommandOperation();
            commandOp.setCode("configranges");
            commandOp.setType(Operation.Type.COMMAND);
            commandOp.setEnabled(true);
            commandOp.setPayLoad(payloadObject.toString());

            log.info("publishing to topic "+ publishTopic+"\n \""+payloadObject.toString()+"\"");
